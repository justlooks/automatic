...
#cat << EOF > /opt/mysqld_exporter-0.10.0.linux-amd64/.my.cnf
#[client]
#user=prometheus
#password=123456
#EOF
...
/opt/mysqld_exporter-0.10.0.linux-amd64/mysqld_exporter -collect.binlog_size=true -collect.info_schema.processlist=true -collect.perf_schema.eventsstatements=true
status=$?
if [ $status -ne 0 ]; then
